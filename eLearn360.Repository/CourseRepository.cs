    ...
    public class CourseRepository : Repository<Guid,Course>, ICourseRepository
    {
            ...
            try
            {
                Course newCourse = await _entities.Where(c => c.Id == courseid)
                                                  ...
                                                  .Select(oc => new Course()
                                                  {
                                                      ...
                                                      CourseHasSections = oc.CourseHasSections.Where(d => d.DeleteDate == null && d.Section.DeleteDate == null).Select(ochs => new CourseHasSection()
                                                      {
                                                          ...
                                                          Section = new Section()
                                                          {
                                                              ...
                                                              CreationTrackingUserId = userid,
                                                              SectionHasLessons = ochs.Section.SectionHasLessons.Where(d => d.DeleteDate == null && d.Lesson.DeleteDate == null).Select(oshl => new SectionHasLesson()
                                                              {
        ...
        public async Task<List<Guid>> GetSectionGuid(Guid courseid)
        {
            try
            {
                List<Guid> sectionGuid = await _entities.Where(s => s.Id == courseid && s.DeleteDate == null)
                                                       ...
                                                       .SelectMany(i => i.CourseHasSections)
                                                       .Where(d => d.DeleteDate == null && d.Section.DeleteDate == null)
        ...
        public async Task<List<CourseHasSection>> GetIncludeCourseHasSection(Guid courseid)
        {
            ...
            try
            {
                List<CourseHasSection> courseHasSections = await _entities.Where(s => s.Id == courseid)
                                                                         ...
                                                                         .SelectMany(chs => chs.CourseHasSections)
                                                                         .Where(d => d.DeleteDate == null && d.Section.DeleteDate == null)
        ...
        public async Task<List<Guid>> GetAllGuidByCourseId(Guid courseId)
        {
            try
            {
                ...
                var Allguids = course.CourseHasSections.Where(x => x.DeleteDate == null && x.Section.DeleteDate == null).DistinctBy(x => x.SectionId).OrderBy(p => p.Position).Select(x => new
                {
                    SectionId = x.SectionId,
                    LessonId = x.Section.SectionHasLessons.Where(x => x.DeleteDate == null && x.Lesson.DeleteDate == null).OrderBy(p => p.Position).DistinctBy(z => z.LessonId).Select(l => l.LessonId).ToList()
