...
namespace eLearn360.Repository
{
    public class GroupRepository : Repository<Guid, Group>, IGroupRepository
    {
            ...
            try
            {
                Group group = (await _entities.Where(g => g.Id == groupId)
                    ...
                    .Select(g => new Group()
                    {
                        Id = g.Id,
                        Name = g.Name,
                        UserHasGroups = g.UserHasGroups.Where(d => d.DeleteDate == null && d.User.DeleteDate == null).SelectMany(u => new List<UserHasGroup>()
                        {
                            new UserHasGroup()
                            {
                                User = new User()
                            ...
                            StaffOccupations = u.StaffOccupations.Where(d => d.DeleteDate == null).Select(st => new StaffOccupation()
                            {
                                Name = st.Name
                            }).ToList(),
        ...
        public async Task<List<Group>> GetGroupsByOrganizationId(Guid OrganizationId)
        {
            try
            {
                List<Group> groups = await _entities.Where(u => u.OrganizationId == OrganizationId && u.DeleteDate == null)
                                                    .ToListAsync();
                return groups;
            }
            catch (Exception ex)
